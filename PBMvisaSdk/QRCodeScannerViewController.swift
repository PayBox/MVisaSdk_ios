...
open class QRCodeScannerViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate
{
    ...
    func loadErrorText(){
        ...
        textError.font = UIFont(name: textError.font.fontName, size: 18)
        textError.text = "Некорректный QR Код"
        textError.textColor = UIColor.red
        textError.textAlignment = NSTextAlignment.center
        textError.frame = CGRect(x: 0, y: 35, width: view.bounds.width, height: 100)
    }
    ...
    public func startQRCodeScanningSession(){
        ...
        textError.textColor = UIColor.red
        textError.frame = CGRect(x: view.bounds.width/2, y: 15, width: 100, height: 40)
